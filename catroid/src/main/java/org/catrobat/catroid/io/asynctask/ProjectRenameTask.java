...
public class ProjectRenameTask extends AsyncTask<Void, Void, Boolean> {
	...
	public static File task(File projectDir, String dstName) throws IOException {
		...
		try {
			if (projectDir.renameTo(dstDir) && XstreamSerializer.renameProject(new File(dstDir, CODE_XML_FILE_NAME),
					dstName)) {
				return dstDir;
			}
		} catch (IOException e) {
			Log.e(TAG, "Cannot rename project directory " + projectDir.getAbsolutePath() + " to " + dstName);
			throw e;
		}
